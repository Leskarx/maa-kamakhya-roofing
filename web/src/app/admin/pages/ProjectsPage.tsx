...
import { Plus, Pencil, Trash2, Images, MapPin, Calendar, FolderOpen, X } from 'lucide-react';
...
const emptyForm: ProjectForm = {
  ...
  completionDate: '', status: 'Completed', description: '', client: '',
};
...
export default function ProjectsPage() {
  ...
  const openAdd = () => { setEditingProject(null); setForm(emptyForm); setFormOpen(true); };
  const openEdit = (p: Project) => {
    setEditingProject(p);
    setForm({ title: p.title, location: p.location, serviceType: p.serviceType, completionDate: p.completionDate, status: p.status, description: p.description, client: p.client });
    setFormOpen(true);
  };
  ...
  const handleSave = () => {
    ...
    if (editingProject) {
      ...
      const newProject: Project = {
        ...
        ...form,
        images: ['https://images.unsplash.com/photo-1763665814538-8ba04597286c?w=800&h=600&fit=crop&auto=format'],
      };
    ...
    <div>
      ...
      {projects.length === 0 ? (
        <EmptyState
          ...
          description="Start adding your roofing projects to showcase your work."
          action={
          ...
          <Dialog.Content className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-full max-w-lg bg-white rounded-2xl shadow-2xl p-6 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <div className="bg-gray-50 border-2 border-dashed border-gray-200 rounded-xl p-4 text-center">
                ...
                <p className="text-xs font-semibold text-gray-500">Image Upload</p>
                <p className="text-[11px] text-gray-400 mt-1">Cloudinary integration — coming soon</p>
              </div>
            </div>
            ...
            <div className="flex gap-3 mt-6 pt-5 border-t border-gray-100">
              <button
                onClick={handleSave}
                className="flex-1 py-2.5 rounded-xl bg-[#0B2E6B] text-white text-sm font-bold hover:bg-[#0a2660] transition-colors"
              >
                {editingProject ? 'Save Changes' : 'Add Project'}
              </button>
              <Dialog.Close asChild>
                <button className="px-5 py-2.5 rounded-xl bg-gray-100 text-gray-700 text-sm font-bold hover:bg-gray-200 transition-colors">
                  Cancel
                </button>
              </Dialog.Close>
            </div>
            ...
            {galleryProject && (
              <>
                <div className="relative">
                  <img
                    src={galleryProject.images[galleryIdx]}
