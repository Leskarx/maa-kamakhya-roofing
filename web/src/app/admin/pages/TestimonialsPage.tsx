...
import { Plus, Pencil, Trash2, Eye, EyeOff, Star as StarIcon, X } from 'lucide-react';
...
export default function TestimonialsPage() {
  ...
  const openEdit = (t: Testimonial) => {
    setEditingId(t.id);
    setForm({ customerName: t.customerName, location: t.location, reviewText: t.reviewText, rating: t.rating, status: t.status });
    setFormOpen(true);
  };

  const toggleStatus = (id: string) => {
    setTestimonials(prev => prev.map(t =>
      t.id === id ? { ...t, status: t.status === 'Published' ? 'Hidden' : 'Published' } : t
    ));
    const t = testimonials.find(t => t.id === id);
    toast.success(t?.status === 'Published' ? 'Testimonial hidden' : 'Testimonial published');
  };
  ...
  const handleSave = () => {
    ...
    const colors = ['#0B2E6B', '#D72626', '#d97706', '#1e40af', '#7c3aed', '#0d9488'];
    if (editingId) {
      setTestimonials(prev => prev.map(t => t.id === editingId ? { ...t, ...form } : t));
      toast.success('Testimonial updated');
    } else {
      const newT: Testimonial = {
        id: `TST-${String(testimonials.length + 1).padStart(3, '0')}`,
        ...form,
        dateAdded: new Date().toISOString().split('T')[0],
        initials: form.customerName.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2),
        avatarColor: colors[testimonials.length % colors.length],
      };
  ...
  return (
    <div>
      <PageHeader
        title="Testimonials"
        subtitle={`${testimonials.filter(t => t.status === 'Published').length} published, ${testimonials.filter(t => t.status === 'Hidden').length} hidden`}
        action={
          <button
      ...
      {testimonials.length === 0 ? (
        <EmptyState
          ...
          description="Add customer reviews to build trust on your website."
          action={<button onClick={openAdd} className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-[#0B2E6B] text-white text-sm font-bold"><Plus className="w-4 h-4" /> Add First Review</button>}
        />
      ) : (
